  ...
  var cycleEvent = function(compilation, cb) {
    ...
    return promises
      ...
      .catch(function(statusCode) {
        console.error('Error: PharseApp API returned status code: ' + statusCode);
        cb();
      })
      .then(function(data) {
        if (!data) return;
        ...
        return locales.reduce(function(accumulator, locale) {
          ...
          return accumulator.then(function() {
            ...
            .catch(function(statusCode) {
              console.error('Error: PharseApp API returned status code: ' + statusCode + ' for request with locale ' + locale.id);
            });
        }, Promise.resolve());
      })
      .then(function() {
